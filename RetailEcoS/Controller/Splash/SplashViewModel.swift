...
class SplashViewModel {
    ...
    var getPhoneDetails: ModelToFetchPhoneCodeDetails!
    ...
    ///mark: function
    ///
    ///mark:  getrequest to get postal code, country name for current lat/long.[GOOGLE API]
    ...
    ///mark: getrequest to get Timezone for present country[GOOGLE API]
    ...
    func getCompanyDetails(postRequest: String,sendDataModel: ModelToGiveCountry,vc : UIViewController, completion: @escaping (String) -> ()) {
    ...
    func getPhoneCodeDetails(postRequest: String,sendDataModel: ModelToGiveCountryForPhoneCode,vc : UIViewController, completion: @escaping (String) -> ()) {
            ...
            try Service.shared.postRequest(urls: postRequest, getModel: jsonData, vc: vc, completion: { result in
                ...
                if let data = result {
                    ...
                    if let jsonString = String(data: data, encoding: .utf8) {
                        ...
                        do {
                            //DECODING JSON STRING OBJECT TO MODEL
                            let getPhoneCode = try JSONDecoder().decode(ModelToFetchPhoneCodeDetails.self, from: jsonData)
                            
                            if getPhoneCode.status.Result == 1{
                                self.getPhoneDetails = getPhoneCode
                                completion("")
                            }else{
                                self.showError.showError(getError: "Invalid Response!", getMessage: "Problem in getting Server Response!")
                            }
